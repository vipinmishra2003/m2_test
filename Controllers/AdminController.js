...
Adminlogin: async (req, res) => {
    try {
        ...
        const foundUser = await faculty.findOne({ $and: [ { $and: [{ email: email }, { role: "ADMIN" }] }, { password: password}]})
        ...
        return res.status(200).json({ message: 'Login Successfull', generatedToken })
    }
    catch (error) {
        console.error(error)
...
getfaculty: async (req, res) => {
    try {
        ...
        const allfaculty = await faculty.find({ $and: [{ role: "USER" }, { status: "ACTIVE" }] })

        return res.status(201).json({ message: "All faculty are fetched successfully", allfaculty })
    }
    catch (error) {
        console.error(error)
...
createCourse: async (req, res) => {
    ...
    try {
        ...
        const existingCourse = await faculty.findOne({ $and: [{ department: department }, { course: course }] })
        if (existingCourse) {
